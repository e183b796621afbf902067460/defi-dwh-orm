...
class PITLendingPoolBorrowOverview(Base):

    __tablename__ = 'pit_lending_pool_borrow_overview'
    __table_args__ = {
        'comment': 'Lending Pool Borrow Overview'
    }

    pit_lending_pool_borrow_overview_id = Column(Integer, primary_key=True)  # PK
    ...
    pit_health_factor = Column(Float, nullable=False)
    pit_lending_pool_borrow_overview_load_ts = Column(DateTime, server_default=func.now(), nullable=False)
